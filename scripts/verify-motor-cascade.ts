...
async function main() {
  ...
  const FAMILIES: any[] = [
    { id: "1", family_key: "magnetic_vector", label: "Magnetic / Vector", domain: "clipper_trimmer_shaver", aliases: ["electromagnetic", "vector", "magnetic"], modifier: false, adjacent_families: ["pivot"], enabled: true, sort_order: 0 },
    { id: "2", family_key: "rotary", label: "Rotary", domain: "clipper_trimmer_shaver", aliases: ["rotary motor"], modifier: false, adjacent_families: [], enabled: true, sort_order: 1 },
    { id: "3", family_key: "pivot", label: "Pivot", domain: "clipper_trimmer_shaver", aliases: ["pivot motor"], modifier: false, adjacent_families: ["magnetic_vector"], enabled: true, sort_order: 2 },
  ];
  ...
  console.log("\n[1] Title scanning — previously not scanned at all");
  {
    ...
    assert(withMotorOnlyInTitle?.familyKey === "magnetic_vector", `motor mentioned only in the title resolves correctly (got ${withMotorOnlyInTitle?.familyKey})`);
  ...
  console.log("\n[2] Existing cascade steps still resolve correctly with confirmedVia labels");
  {
    ...
    assert(descMatch?.familyKey === "magnetic_vector" && descMatch?.confirmedVia === "description", `description-sentence match labeled "description" (got ${descMatch?.confirmedVia})`);
  ...
  const BRANDED_NAMES: any[] = [
    { id: "b1", brand_name: "Wahl", branded_term: "IN3", family_key: "magnetic_vector", enabled: true, sort_order: 0 },
  ...
  ];
  {
    ...
    assert(matched?.familyKey === "magnetic_vector", `"IN3" resolves to Magnetic/Vector for Wahl (got ${matched?.familyKey})`);
  ...
  console.log("\n[4] extractCompetitorMotorType checks the branded map FIRST, before generic aliases");
  {
    ...
    assert(result?.familyKey === "magnetic_vector" && result?.confirmedVia === "branded_map", `branded map resolves through the full extraction cascade (got familyKey=${result?.familyKey}, confirmedVia=${result?.confirmedVia})`);
