...
- "${productName}" news
- ${brandLine} launch OR recall OR update OR award ${year}
...
export async function findProductNews(productName: string, brand: string | null, referenceDate: Date = new Date()): Promise<ProductNewsResult> {
  ...
  if (!hasOpenAIKey) {
    return { items: [], categoryContext: [], searchedAt, aiUnavailable: true };
  }

  const { systemPrompt, userPrompt } = buildPrompt(productName, brand, referenceDate);
  ...
  try {
    ...
    if (!text || annotations.length === 0) {
      ...
      return { items: [], categoryContext: [], searchedAt, aiUnavailable: !text };
    }
    ...
    return { items, categoryContext: [], searchedAt, aiUnavailable: false };
  } catch (err) {
