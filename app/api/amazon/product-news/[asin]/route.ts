...
// query param, same aiUnavailable-never-cached rule.
const NEWS_TTL_MS = 24 * 60 * 60 * 1000;

async function getCachedNews(asin: string): Promise<{ result: ProductNewsResult; fetchedAt: string } | null> {
...
async function setCachedNews(asin: string, result: ProductNewsResult) {
  if (!isSupabaseConfigured) return;
  try {
    await supabaseAdmin
      .from("amazon_cache")
      .upsert(
        { asin, cache_type: "product_news", payload: result, fetched_at: new Date().toISOString() },
...
export async function GET(req: NextRequest, { params }: { params: { asin: string } }) {
  ...
  try {
    if (!forceRefresh) {
      const cached = await getCachedNews(asin);
      if (cached) {
    ...
    if (!result.aiUnavailable) {
      await setCachedNews(asin, result);
    }
